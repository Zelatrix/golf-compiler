...
class CodeGen(Visitor):
    ...
    def visit_while(self, predicate, body):
        ...
        p = self.visit(predicate)  # Work out the loop predicate
        bool_cond = self.builder.fptosi(p, ir.IntType(1))  # Convert the predicate to a binary integer

        with self.builder.if_then(bool_cond) as then:
            # Recalculate the predicate each time through the loop
            p = self.visit(predicate)   
            new_cond = self.builder.fptosi(p, ir.IntType(1))  
            
            # Evaluate each statement in the loop body
            for stmt in body:
                self.visit(stmt) 
            self.builder.branch(loop_entry) # Jump back to start of the loop  
